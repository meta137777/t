...
const getBase64Images = async (image_guids: string) => {
  // Check if the parameters is non-null and is a string.
  if (image_guids && typeof image_guids === 'string') {
    let imageArray = image_guids.split(',').filter((img) => Boolean(img));
    if (imageArray && Array.isArray(imageArray)) {
      ...
      throw new Error('Image array is not iterable.')
    }
  }
};
